...
public class Client {
    public static void main(String[] args) {
        ...
        AuthenticationHandler chain = new AdministratorAuthenticationHandler(adminsAccounts);
        chain.linkWith(new UserAuthenticationHandler(usersAccounts))
                .linkWith(new GuestAuthenticationHandler());

        for (Account user : users) {
            System.out.println(chain.authenticate(user));
        }
    }
}
